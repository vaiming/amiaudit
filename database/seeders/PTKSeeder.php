...
class PTKSeeder extends Seeder
{
  ...
  public function run()
  {
    ...
    foreach ($ptks as $unitKerjaId => $ruangLingkups) {
      foreach ($ruangLingkups as $item) {
        ...
        if ($riwayatAudit !== null) {
          ...
          if ($riwayatAudit->checklist_audits->isNotEmpty() && \Arr::random([true, true, true, false])) {
            $checklist = $riwayatAudit->checklist_audits->random();

            if ($checklist->ptk === null) {
              $ptkCreated = PTK::factory(1)->create([
                'type' => \Arr::random(['oai', 'ptk']),
                'auditor_id' => $auditors->random()->id ?? null,
                'auditee_id' => $auditees->random()->id ?? null,
                'checklist_audit_id' => $checklist->id,
                'riwayat_audit_id' => $riwayatAudit->id,
                'penanggung_jawab_perbaikan' => $unitKerja->id,
              ]);

              ChecklistAudit::find($checklist->id)->update([
                'is_approved_by_admin' => true,
                'is_approved_by_auditor' => true,
                'is_marked_as_audit_completed' => true,
                'is_marked_as_ptk' => true,
              ]);

              // Completing PTK/OAI
              if (\Arr::random([true, false])) {
                $ptkCreated->first()->update([
                  'is_completed' => true,
                  'is_approved_by_auditee' => true,
                  'is_approved_by_auditor' => true,
                  'is_approved_with_repaired_by_auditor' => true,
                ]);
              }
            }
          }
