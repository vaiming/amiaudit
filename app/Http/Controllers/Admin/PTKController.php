...
class PTKController extends Controller
{
  ...
  public function create(UnitKerja $unitKerja, RiwayatAudit $riwayatAudit, ChecklistAudit $checklistAudit)
  {
    if (
      $riwayatAudit->ptks()
        ->where('checklist_audit_id', $checklistAudit->id)
        ->count()
    ) {
      return redirect()->route('admin.ptk.riwayat-audit.show', [
    ...
    return view('admin.ptk.create', compact(
      'unitKerja',
      'riwayatAudit',
      'standarKriterias',
      'unitKerjas',
  ...
  )
  {
    ...
    return redirect(route('admin.ptk.show', [
  ...
  )
  {
    ...
    $riwayatAudit->load(['ruang_lingkup', 'auditee']);
  ...
  )
  {
    ...
    return view('admin.ptk.edit', compact(
      'unitKerja',
      'riwayatAudit',
      'checklistAudit',
      'ptk',
  ...
  )
  {
    ...
    return redirect(route('admin.ptk.riwayat-audit.show', [
      ...
      "status" => "success",
      "message" => "PTK dengan standar kriteria <strong>{$ptk->checklist_audit->standarKriteria->nama}</strong> dengan measure <em>{$ptk->checklist_audit->measure->measure}</em> berhasil dihapus"
    ]);
  }

  /**
   * @param RiwayatAudit $riwayatAudit
   * @return ChecklistAudit[]|\Illuminate\Database\Eloquent\Collection|\LaravelIdea\Helper\App\Models\_IH_ChecklistAudit_C
   */
