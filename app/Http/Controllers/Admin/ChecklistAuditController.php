...
class ChecklistAuditController extends Controller
{
  ...
  public function create(UnitKerja $unitKerja, RiwayatAudit $riwayatAudit)
  {
    ...
    return view('admin.checklist-audit.create', compact(
      'unitKerja',
      'riwayatAudit',
      'standarKriterias',
      'auditees',
  ...
  )
  {
    ...
    return redirect(route('admin.checklist-audit.show', [
  ...
  )
  {
    ...
    $checklistAudits = $riwayatAudit->checklist_audits()
      ->with(['standarKriteria', 'indikator', 'measure', 'auditee', 'auditor'])
      ->get();
    ...
    $haveTemuan = $checklistAudit->langkah_kerja_checklists()
      ->get(['is_audited'])->pluck('is_audited')
      ->contains(false);
  ...
  )
  {
    ...
    return view('admin.checklist-audit.edit', compact(
      'unitKerja', 'riwayatAudit', 'checklistAudit',
      'standarKriterias', 'pernyataans', 'indikators', 'measures',
      'auditees', 'auditors',
    ));
  }
  ...
  )
  {
    ...
    $checklistAudit->is_approved_by_admin = true;
  ...
  )
  {
    ...
    return redirect(url()->previous() . '#checklist-audits')->with([
      "status" => "success",
      "message" => "Checklist audit ini telah ditandai memiliki Temuan"
    ]);
  }
  ...
  )
  {
    ...
    return redirect(url()->previous() . '#checklist-audits')->with([
      "status" => "success",
      "message" => "Checklist audit ini telah ditandai tidak memiliki Temuan"
    ]);
  }
  ...
  )
  {
    $pernyataans = $unitKerja->pernyataanStandarUnitKerjas()
      ...
      ->get()->load(['pernyataan_standar'])->pluck('pernyataan_standar');
  ...
  )
  {
    ...
    if (\request()->routeIs('admin.checklist-audit.fetchIndikatorMeasurePernyataanStandarKriteriaEdit')) {
  ...
  )
  {
    ...
    return redirect(route('admin.checklist-audit.riwayat-audit.show', [
        $unitKerja->id, $riwayatAudit->id
      ]) . '#checklist-audits')->with([
      "status" => "success",
      "message" => "Checklist audit dengan standar kriteria <strong>{$checklistAudit->standarKriteria->nama}</strong> dengan measure <strong>{$checklistAudit->measure->measure}</strong> berhasil dihapus"
    ]);
  }
}
